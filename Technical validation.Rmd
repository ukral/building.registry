...
# Import datasets
In order to run the code, please set your working directory at code line 42.
...
#setwd(dir = "C:/Users/u.kral/Documents")
######################################################################################

# download the files from github repository
#download.file(url = "https://github.com/ukral/building.registry/raw/master/Dataset.csv", destfile = "Dataset.csv")
#download.file(url = "https://github.com/ukral/building.registry/raw/master/Dataset.csv", destfile = "Dataset.csv")



######################################################################################
path <- "C:/Users/u.kral/ownCloud/03_TU Wien/Github/historic.building.registry.vienna/"
######################################################################################
...
cadastral_raw <- read.csv(file=paste(path, "Data.descriptor_files/Online-only Table 2.csv", sep=""),  sep = ";" , stringsAsFactors = F)
cadastral <- cadastral_raw[1:66,]

# Import file "adressen_standorte_wien_20201015.csv". This file includes today's street names. The is available on the Open Govoernment Data plattforma and can be retrieved from https://www.data.gv.at/katalog/dataset/stadt-wien_adressdatenderstadtwien. We retrieved the file at 15 October 2020 and copied it the the Github repository.
adressen <- read.csv(file=paste(path, "Data.descriptor_files/adressen_standorte_wien_20201015.csv", sep=""), sep = ";", stringsAsFactors = F,fileEncoding = "UTF-8")


# Import file "statistical_yearbook (1914).xlsx"
floors_1914 <- data.frame(read_xlsx(paste(path, "Data.descriptor_files/statistical_yearbook (1914).xlsx", sep=""), sheet = "STKW_hist", col_names = F, range = "B7:I27"), stringsAsFactors = F)
...
yearbook_1923 <- read_xlsx(paste(path, "Data.descriptor_files/statistical_yearbook (1923).xlsx", sep=""), sheet = "Rohdaten", col_names =  TRUE, col_types = rep("numeric", times = 6))
...
length(levels(as.factor(dataset$str_ma37.y))) # 2587 werden gefunden

match_yes<- dataset[which(dataset$str_ma37.y != ""),] # nrow 41195 gefunden, no change
match_no <- dataset[which(is.na(dataset$str_ma37.y) == T),] # 1666 nicht gefunden

match_yes_control <- dataset[which(dataset$str_1920_norm == dataset$str_2010_norm), ] # 40489
match_no_control <- dataset[which(dataset$str_1920_norm != dataset$str_2010_norm), ] # 2372 f: 281 bei STR.1920s.


# 2010 vs. 1920. Die Summe der Einträge ist 2372 Passt mit match_no_control überein.
# test 1 und test 2: Schreibweise durch Adressen Standorte Wien bestätigt. 580+129 = 709
# test 3 und test 4: Schreibweise durch Adressen Standorte Wien NICHT bestätigt. 1405+258 = 1663
...
# Diese Zahlen kommen in den Data Descriptor
...
str_2010_1 <- dataset[which(is.na(dataset$str_ma37.x) == FALSE),] # diese wurden alle gefunden
str_2010_2 <- dataset[which(is.na(dataset$str_ma37.x) == TRUE),] # STR.2010 ist leer
#str_2010_3 <- which((control2$str_2010_norm) != "") # STR.2010 vorhanden, aber mit Adressen Standorte Wien nicht verifiziert. Dafür mit Online-Stadtplen
...
# Seitenanzahl per BEZ
totals<- kg1[,c("UD.1920s", "PDF.page.nr")]
#totals$UD.1920s <- as.integer(totals$UD.1920s)
#totals <- totals[order(totals$UD.1920s),]
#totals$UD.1920s <- as.character(totals$UD.1920s)
...
# "Nach der Schaffung des Bundeslands Wien (29. Dezember 1921 mit Wirksamkeit vom 1. Jänner 1922) kam es zu weiteren Grenzänderungen
## 22. Dezember 1923: 2./20./21. Bezirk
## 13. Juni 1930: 10./12. Bezirk
## 14. Dezember 1932: 13./16. Bezirk
## 17. Oktober 1935: 16./17. Bezirk)." Quelle: https://www.geschichtewiki.wien.gv.at/Bezirke
...
## cd2 plotten
temp <- cbind(cd2[,2:3], yearbook_1923$Häuser_3)
...
#floors <- floors[order(floors$FLOORS.1920s),]
